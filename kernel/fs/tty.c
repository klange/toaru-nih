...
uint32_t  read_pty_master(fs_node_t * node, koff_t offset, uint32_t size, uint8_t *buffer) {
	pty_t * pty = (pty_t *)node->device;

	/* Standard pipe read */
	return ring_buffer_read(pty->out, size, buffer);
}
uint32_t write_pty_master(fs_node_t * node, koff_t offset, uint32_t size, uint8_t *buffer) {
	pty_t * pty = (pty_t *)node->device;
...
uint32_t  read_pty_slave(fs_node_t * node, koff_t offset, uint32_t size, uint8_t *buffer) {
	pty_t * pty = (pty_t *)node->device;
...
uint32_t write_pty_slave(fs_node_t * node, koff_t offset, uint32_t size, uint8_t *buffer) {
	pty_t * pty = (pty_t *)node->device;
