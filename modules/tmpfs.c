...
static uint32_t read_tmpfs(fs_node_t *node, koff_t offset, uint32_t size, uint8_t *buffer) {
	struct tmpfs_file * t = (struct tmpfs_file *)(node->device);
	...
	if (start_block == end_block) {
		...
		memcpy(buffer, (uint8_t *)(((uint32_t)buf) + ((uint32_t)offset % BLOCKSIZE)), size_to_read);
		...
		for (block_offset = start_block; block_offset < end_block; block_offset++, blocks_read++) {
			if (block_offset == start_block) {
				...
				memcpy(buffer, (uint8_t *)(((uint32_t)buf) + ((uint32_t)offset % BLOCKSIZE)), BLOCKSIZE - (offset % BLOCKSIZE));
				spin_unlock(tmpfs_page_lock);
			} else {
				void *buf = tmpfs_file_getset_block(t, block_offset, 0);
				memcpy(buffer + BLOCKSIZE * blocks_read - ((uint32_t)offset % BLOCKSIZE), buf, BLOCKSIZE);
				spin_unlock(tmpfs_page_lock);
			}
		}
		if (end_size) {
			void *buf = tmpfs_file_getset_block(t, end_block, 0);
			memcpy(buffer + BLOCKSIZE * blocks_read - ((uint32_t)offset % BLOCKSIZE), buf, end_size);
			spin_unlock(tmpfs_page_lock);
		}
	}
	return size_to_read;
...
static uint32_t write_tmpfs(fs_node_t *node, koff_t offset, uint32_t size, uint8_t *buffer) {
	struct tmpfs_file * t = (struct tmpfs_file *)(node->device);
	...
	if (start_block == end_block) {
		...
		memcpy((uint8_t *)(((uint32_t)buf) + ((uint32_t)offset % BLOCKSIZE)), buffer, size_to_read);
		spin_unlock(tmpfs_page_lock);
		return size_to_read;
	} else {
		uint32_t block_offset;
		...
		for (block_offset = start_block; block_offset < end_block; block_offset++, blocks_read++) {
			if (block_offset == start_block) {
				...
				memcpy((uint8_t *)(((uint32_t)buf) + ((uint32_t)offset % BLOCKSIZE)), buffer, BLOCKSIZE - (offset % BLOCKSIZE));
				spin_unlock(tmpfs_page_lock);
			} else {
				void *buf = tmpfs_file_getset_block(t, block_offset, 1);
				memcpy(buf, buffer + BLOCKSIZE * blocks_read - ((uint32_t)offset % BLOCKSIZE), BLOCKSIZE);
				spin_unlock(tmpfs_page_lock);
			}
		}
		if (end_size) {
			void *buf = tmpfs_file_getset_block(t, end_block, 1);
			memcpy(buf, buffer + BLOCKSIZE * blocks_read - ((uint32_t)offset % BLOCKSIZE), end_size);
			spin_unlock(tmpfs_page_lock);
		}
	}
	return size_to_read;
