...
static uint32_t read_server(fs_node_t * node, koff_t offset, uint32_t size, uint8_t * buffer) {
...
static uint32_t write_server(fs_node_t * node, koff_t offset, uint32_t size, uint8_t * buffer) {
	pex_ex_t * p = (pex_ex_t *)node->device;
...
static uint32_t read_client(fs_node_t * node, koff_t offset, uint32_t size, uint8_t * buffer) {
...
static uint32_t write_client(fs_node_t * node, koff_t offset, uint32_t size, uint8_t * buffer) {
	pex_client_t * c = (pex_client_t *)node->inode;
	if (c->parent != node->device) {
