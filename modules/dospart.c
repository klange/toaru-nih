...
static uint32_t read_part(fs_node_t *node, koff_t offset, uint32_t size, uint8_t *buffer) {
...
static uint32_t write_part(fs_node_t *node, koff_t offset, uint32_t size, uint8_t *buffer) {
	struct dos_partition_entry * device = (struct dos_partition_entry *)node->device;
